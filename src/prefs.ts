...
} from "./ui/theme_consistency/apply.js";

type PreferencesDialogLike = Pick<Adw.PreferencesDialog, "add"> &
  Partial<
    Pick<
      Adw.PreferencesDialog,
      "set_content_width" | "set_content_height" | "set_presentation_mode"
    >
  > & {
...
export default class OTilingPreferences extends ExtensionPreferences {
  async fillPreferencesWindow(window: PreferencesDialogLike) {
    const settings = this.getSettings();

    // Compact fixed-size dialog, close button only (no minimize/maximize)
    if (window.set_content_width && window.set_content_height) {
      window.set_content_width(720);
    ...
    themesConsistencyRow.connect("notify::selected", () => {
      ...
      const style =
        selected === 0 ? "default" : selected === 1 ? "rounded" : "sharp";
    ...
    themesConsistencyRow.selected =
      currentStyle === "rounded" ? 1 : currentStyle === "sharp" ? 2 : 0;
    ...
    const switcherStyleRow = new Adw.SwitchRow({
      ...
      subtitle: _(
        "Customize the appearance and auto-scaling of the workspace thumbnails strip",
      ),
    });
    overviewGroup.add(switcherStyleRow);
    settings.bind(
      "workspace-switcher-style",
      switcherStyleRow as any,
      "active",
      Gio.SettingsBindFlags.DEFAULT,
    );
    ...
    settings.bind(
      "panel-transparency-opacity",
      panelOpacityRow as any,
      "value",
      Gio.SettingsBindFlags.DEFAULT,
    );
    ...
    const updatePanelTopGapVisibility = () => {
      const fullyTransparent =
        panelTransRow.active && panelOpacityRow.value === 0;
    ...
    const activeHint = new Adw.SwitchRow({
      ...
      subtitle: _(
        "Highlight the currently focused window with a colored border",
      ),
    });
    ...
    settings.bind(
      "active-hint-overlay-opacity",
      overlayOpacity as any,
      "value",
      Gio.SettingsBindFlags.DEFAULT,
    );
    ...
    const useOverlayColor = new Adw.SwitchRow({
      ...
      subtitle: _(
        "Override default active border color for the window background tint overlay",
      ),
    });
    ...
    const overlayAll = new Adw.SwitchRow({
      ...
      subtitle: _(
        "Render the background overlay tint on all tiled windows on the workspace, instead of only the active window",
      ),
    });
    auraOverlayGroup.add(overlayAll);
    settings.bind(
      "active-hint-overlay-all-windows",
      overlayAll as any,
      "active",
      Gio.SettingsBindFlags.DEFAULT,
    );

    const currentOverlayVal = settings.get_string(
      "active-hint-overlay-color-rgba",
    );
    ...
    useOverlayColor.connect("notify::active", () => {
      ...
      if (active) {
        settings.set_string(
          "active-hint-overlay-color-rgba",
          overlayColorButton.rgba.to_string(),
        );
      } else {
    ...
    this._addShortcutRow(
      tilingGroupShortcuts,
      settings,
      "toggle-tiling",
      _("Toggle Auto-Tiling"),
    );
    this._addShortcutRow(
      tilingGroupShortcuts,
      settings,
      "tile-enter",
      _("Enter Management Mode"),
    );
    this._addShortcutRow(
      tilingGroupShortcuts,
      settings,
    ...
    this._addShortcutRow(
      ...
      settings,
      "tile-orientation",
      _("Toggle Orientation"),
    );
    this._addShortcutRow(
      tilingGroupShortcuts,
      settings,
      "toggle-stacking-global",
      _("Toggle Stacking (Global)"),
    );
    this._addShortcutRow(
      tilingGroupShortcuts,
      settings,
    ...
    const workspaceGroup = new Adw.PreferencesGroup({
      ...
      description: _(
        "Shortcuts for moving windows between workspaces and monitors",
      ),
    });
    ...
    this._addShortcutRow(
      ...
      settings,
      "pop-workspace-up",
      _("Move to Upper Workspace"),
    );
    this._addShortcutRow(
      workspaceGroup,
      settings,
      "pop-workspace-down",
      _("Move to Lower Workspace"),
    );
    this._addShortcutRow(
      workspaceGroup,
      settings,
      "pop-monitor-left",
      _("Move to Left Monitor"),
    );
    this._addShortcutRow(
      workspaceGroup,
      settings,
      "pop-monitor-right",
      _("Move to Right Monitor"),
    );
    this._addShortcutRow(
      workspaceGroup,
      settings,
      "pop-monitor-up",
      _("Move to Upper Monitor"),
    );
    this._addShortcutRow(
      workspaceGroup,
      settings,
      "pop-monitor-down",
      _("Move to Lower Monitor"),
    );
