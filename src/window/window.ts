...
function clutter_focus_is_shell_panel(): boolean {
  try {
    ...
    if (typeof (focused_actor as any).get_meta_window === "function")
      return false;
...
export class ShellWindow {
  ...
  constructor(entity: Entity, window: Meta.Window, window_app: any, ext: Ext) {
    ...
    if ((this.meta.get_compositor_private() as any)?.get_stage())
  ...
  actor_exists(): boolean {
    return (
      !this.destroying && (this.meta.get_compositor_private() as any) !== null
    );
  }
  ...
  private update_hint_colors() {
    ...
    const overlay_base =
      overlay_color_val === "auto" ? color_value : overlay_color_val;
    ...
    if (this.ext.overlay) {
      ...
      this.ext.overlay.set_style(
        `background: ${final_color}; border-radius: ${radius_value}px;`,
      );
    }
  ...
  is_tilable(ext: Ext): boolean {
    return (
      !ext.contains_tag(this.entity, Tags.Floating) &&
      this.is_eligible_for_tiling(ext)
    );
  }
  ...
  same_workspace() {
    ...
    if (workspace) {
      ...
      return (
        workspace_id ===
        (global as any).workspace_manager.get_active_workspace_index()
      );
    }
    return false;
  }
   ...
   */
  restack(
    _updateState: RESTACK_STATE = RESTACK_STATE.NORMAL,
...
function pointer_in_work_area(): boolean {
  ...
  const indice =
    (global as any).backend.get_current_logical_monitor()?.get_number() ?? 0;
  const mon = (global as any).workspace_manager
...
function place_pointer_on(ext: Ext, win: Meta.Window) {
  ...
  const key = Object.keys(focus.FocusPosition)[
    ext.settings.mouse_cursor_focus_location()
  ];
  const pointer_position_ =
