...
function settings_new_schema(schema: string): Settings {
  ...
  const schemaSource =
    schemaDir.query_exists(null) && defaultSource
  ...
  if (!schemaObj) {
    throw new Error(
      "Schema " +
        schema +
        " could not be found for extension o-tiling. Please check your installation.",
    );
  }
...
export class ExtensionSettings {
  ...
  focus_change_on_pointer_rest(): boolean {
    return this.mutter
      ? this.mutter.get_boolean("focus-change-on-pointer-rest")
      : false;
  }
