...
export class Indicator {
  ...
  constructor(ext: Ext) {
    ...
    ext.button_gio_icon_auto_on = Gio.icon_new_for_string(
      `${path}/icons/o-tiling-auto-on-symbolic.svg`,
    );
    ext.button_gio_icon_auto_off = Gio.icon_new_for_string(
      `${path}/icons/o-tiling-auto-off-symbolic.svg`,
    );
  ...
  update_workspace_tiling_state() {
    ...
    if (ext && this.toggle_workspace_tiled) {
      ...
      if (this.toggle_left_pin) {
        let is_pinned = false;
        if (ext.auto_tiler) {
          const toplevel_entity = ext.auto_tiler.forest.find_toplevel([
            monitor,
            workspace,
          ]);
          if (toplevel_entity) {
        ...
        if (this.toggle_left_pin.updateIcon)
          this.toggle_left_pin.updateIcon(is_pinned);
      }
      ...
      if (this.presets_item) {
        if (ext.auto_tiler) {
          const workspace_windows = Array.from(ext.windows.values()).filter(
            (w) =>
              w.known_workspace === workspace &&
              ext.auto_tiler!.attached.contains(w.entity),
          );
          const enabled =
            workspace_windows.length >= 2 && workspace_windows.length <= 6;
...
function tiled(ext: Ext): any {
  ...
  return toggle(
    ...
    "view-grid-symbolic",
    (shouldEnable) => {
      if (ext._indicator_updating) return;
      if (shouldEnable) {
        ext.ext_soft_enable();
      } else {
